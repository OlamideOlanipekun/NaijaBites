...
import GalleryManager from './pages/admin/GalleryManager.tsx';
import SubscriberManager from './pages/admin/SubscriberManager.tsx';
import TestimonialManager from './pages/admin/TestimonialManager.tsx';
import MessageManager from './pages/admin/MessageManager.tsx';
...
const AppContent: React.FC = () => {
  ...
  // Sync router location with currentPage state (legacy support)
  useEffect(() => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items;
  }, [menuItems, activeCategory, searchQuery, sortBy, activeTags]);

  const renderContent = () => {
    // Check if it's an admin route
    if (location.pathname.startsWith('/admin')) {
      return null; // Handled by Routes in App component
    }
                    ...
                    <div className="flex items-center gap-3 overflow-x-auto pb-2 xl:pb-0 no-scrollbar w-full xl:w-auto">
                      {categories.map((cat, idx) => (
                        <button
                          ...
                          className={`whitespace-nowrap px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all border-2 ${activeCategory === cat
                ...
                {menuLoading ? (
                  <div className="flex justify-center items-center py-20">
                    <div className="text-center">
                      ...
                      <p className="text-gray-600 font-medium">Loading delicious dishes...</p>
                    </div>
                  </div>
                ) : menuError ? (
                  <div className="text-center py-20">
                    <div className="text-red-600 text-xl mb-4">⚠️ {menuError}</div>
                    <button
                      onClick={() => window.location.reload()}
                      className="bg-green-700 text-white px-8 py-3 rounded-2xl font-black hover:bg-green-800 transition"
                    >
                      Reload Page
                    </button>
                  </div>
                  ...
                  <div className="text-center py-20">
                    <p className="text-gray-500 text-xl">No dishes found matching your criteria.</p>
                  </div>
                ) : (
      ...
      default:
        ...
        return (
          <div className="animate-fade-in pb-32 md:pb-0">
            ...
            <Stats />
            <FeaturedMenu onAdd={addToCart} onSeeFull={() => navigateTo('menu')} onOpenDetail={setSelectedDish} menuItems={menuItems} />
  ...
  return (
    <div className="min-h-screen selection:bg-yellow-200 selection:text-green-950 flex flex-col font-sans">
      ...
      {!location.pathname.startsWith('/admin') && <Footer onNavigate={navigateTo} />}
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/admin/subscribers" element={<AdminLayout><SubscriberManager /></AdminLayout>} />
        <Route path="/admin/testimonials" element={<AdminLayout><TestimonialManager /></AdminLayout>} />
        <Route path="/admin/messages" element={<AdminLayout><MessageManager /></AdminLayout>} />
...
const FeaturedMenu: React.FC<{
  onAdd: (d: Dish) => void,
  onSeeFull: () => void,
  onOpenDetail: (d: Dish) => void,
  menuItems: Dish[]
}> = ({ onAdd, onSeeFull, onOpenDetail, menuItems }) => (
  <section className="py-32 bg-gray-50/30">
    <div className="max-w-7xl mx-auto px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
        {/* Show loading state or error on homepage */}
        {menuItems.slice(0, 3).map((dish, idx) => (
...
const Newsletter: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { subscribeNewsletter } = await import('./services/api');
      await subscribeNewsletter(email);
      setStatus('success');
      setMessage('Welcome to the Pepper Gang! 🔥');
      setEmail('');
    } catch (err: any) {
      setStatus('error');
  ...
  return (
    <section className="py-24 bg-green-950 text-white relative overflow-hidden mx-6 md:mx-16 rounded-[4rem] mb-24 shadow-[0_50px_100px_-20px_rgba(6,45,29,0.5)]">
      ...
      <div className="max-w-5xl mx-auto px-6 text-center relative z-10">
        ...
        <ScrollReveal animation="reveal" delay={300}>
          {status === 'success' ? (
            <div className="bg-green-800/50 border border-green-600/50 rounded-3xl p-8 max-w-xl mx-auto backdrop-blur-xl">
              <i className="fas fa-check-circle text-4xl text-yellow-500 mb-4"></i>
              <p className="text-xl font-black">{message}</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-5 max-w-3xl mx-auto p-2 bg-white/5 backdrop-blur-2xl rounded-[3rem] border border-white/10 shadow-inner">
              ...
              <button
                type="submit"
                disabled={loading}
                className="bg-yellow-500 hover:bg-white text-green-950 px-12 py-6 rounded-[2.5rem] font-black uppercase tracking-[0.2em] text-sm transition-all shadow-2xl hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"
              >
                {loading ? <i className="fas fa-spinner fa-spin"></i> : <>Join the Gang <i className="fas fa-bolt text-xs"></i></>}
              </button>
            </form>
          )}
          {status === 'error' && (
            <p className="text-red-400 mt-4 font-bold">{message}</p>
          )}
        </ScrollReveal>
