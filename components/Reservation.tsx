...
const ScrollReveal: React.FC<{ 
  ...
  return (
    <div ref={ref} className={`${animation} ${isVisible ? 'active' : ''} ${className}`} style={{ transitionDelay: `${delay}ms` }}>
      {children}
    </div>
  );
};
          ...
          <div className="lg:col-span-5 pt-10">
            ...
            <div className="grid grid-cols-1 gap-8">
              {[
                ...
                { icon: 'fa-utensils', title: 'Special Occasions', desc: 'Celebrating? Mention it in your booking for a special Naija Bites surprise.', color: 'text-green-700', bg: 'hover:bg-green-700' }
              ].map((item, idx) => (
                <ScrollReveal key={idx} animation="reveal-left" delay={idx * 150 + 200}>
                  <div className="flex items-start gap-6 group">
                    <div className={`w-14 h-14 bg-white shadow-xl rounded-2xl flex items-center justify-center ${item.color} shrink-0 ${item.bg} group-hover:text-white transition-all`}>
              ...
              <div className="bg-white p-8 md:p-14 rounded-[3rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] border border-gray-100 relative">
                {submitted ? (
                  <div className="text-center py-20 animate-fade-in">
                    ...
                    <button onClick={() => setSubmitted(false)} className="text-green-700 font-black uppercase tracking-widest hover:underline">
