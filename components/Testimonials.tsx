...
const Testimonials: React.FC = () => {
  ...
  const next = useCallback(() => {
    setTestimonials(prev => {
      ...
      setActiveIdx(current => (current + 1) % prev.length);
      return prev;
    });
  }, []);
  ...
  const prev = useCallback(() => {
    setTestimonials(prev => {
      if (prev.length === 0) return prev;
      setActiveIdx(current => (current - 1 + prev.length) % prev.length);
      return prev;
    });
  }, []);
        ...
        <div className="max-w-4xl mx-auto relative px-4 md:px-12">
          ...
          >
            {testimonials.map((t, idx) => (
              <div
                key={t.id || idx}
                className={`transition-all duration-700 absolute w-full ${idx === activeIdx
                ...
                <div className="bg-white rounded-[3rem] p-8 md:p-16 shadow-2xl relative">
                  ...
                  <div className="flex flex-col items-center md:items-start md:flex-row gap-10">
                    <div className="shrink-0 relative">
                      <img src={t.avatar_url || 'https://via.placeholder.com/150'} className="w-24 h-24 md:w-32 md:h-32 rounded-3xl object-cover border-4 border-yellow-500/20 shadow-lg" alt={t.name} />
        ...
        <div className="flex justify-center gap-3 mt-16">
          {testimonials.map((_, i) => (
            <button
              ...
              className={`h-2 transition-all duration-300 rounded-full ${i === activeIdx ? 'w-10 bg-yellow-500' : 'w-2 bg-white/20 hover:bg-white/40'
                }`}
            />
          ))}
        </div>
